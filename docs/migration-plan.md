...
| **Phase 2: Collaborative Alignment & Brainstorming** | `core/goal-contract.md` | **MODIFY & KEEP** | Upgraded from informal conversational alignment to a binding, machine-verifiable [Goal Contract](../core/goal-contract.md) containing explicit Acceptance Criteria, constraints, and out-of-scope declarations. |
...
1. **Elimination of Self-Certification**: In the legacy workflow, the same agent that wrote code decided it was done. In the new architecture, only the independent [Judge Agent](../agents/judge.md) can grant a `PASS` verdict.
2. **Pre-MR Adversarial Inspection**: Flaws are caught and fixed locally via the [Devil's Advocate](../agents/devil-advocate.md) *before* publishing PRs/MRs to remote repositories.
3. **No-Progress & Escalation Circuit Breakers**: Bounded iterations (`MAX_ITERATIONS = 3`) and [No-Progress Detection](../policies/no-progress-policy.md) prevent runaway loops and thrashing.
