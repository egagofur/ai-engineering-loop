...
The deterministic recipe builder scaffolds from safe presets, validates candidates before atomic installation, and preserves replaced versions locally. Workflow Studio is a local interface over that compiler and controlled runtime: first-class Edges, contextual node creation, inline Display Names with stable Technical IDs, multi-select, guarded JSON import/export, prompt-configured Custom Agents, Goal Freeze/versioning, human-readable Runs, searchable read-only Run History, bounded evidence inspection, and trigger-local execution of the AEL runtime. It never executes a model or shell command in the browser and never claims dispatch into an external AI conversation. See [`core/local-workflow-studio.md`](core/local-workflow-studio.md). The host-neutral agent protocol remains available in [`agents/recipe-builder.md`](agents/recipe-builder.md).
