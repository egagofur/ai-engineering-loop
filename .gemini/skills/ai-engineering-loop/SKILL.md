...
1. Stage 0: sync hosts, check status, then run `npx ai-engineering-loop run` to create or resume the ledger. Read glossary, lessons, and workflow overlays. Required stages cannot be skipped.
...
3. Stages 2–4: Resolve maker_intern only through the host catalog; otherwise parent is Maker. Intern cannot skip gates. Use red repro or TDD, write `RUN_DIR/diff.patch`, then run `npx ai-engineering-loop gate maker`.
