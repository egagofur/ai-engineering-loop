...
const DEFAULT_CONTEXT_IGNORE_PATTERNS = Object.freeze([
  '*.css',
  '*.map',
  '*.min.js',
  'package-lock.json',
...
function readContextIgnorePatterns(rootDir) {
  const ignorePath = path.join(rootDir, '.aelcontextignore');
...
function matchesContextIgnorePattern(relativePath, pattern) {
  const candidate = normalizeRepoPath(relativePath);
  ...
  if (!normalizedPattern.includes('/')) {
    ...
    return normalizedPattern.includes('*')
      ? globToRegExp(normalizedPattern).test(basename)
      : basename === normalizedPattern;
  }
  return normalizedPattern.includes('*')
    ? globToRegExp(normalizedPattern).test(candidate)
    : candidate === normalizedPattern;
}

function contextIgnorePatterns(rootDir) {
  return [...DEFAULT_CONTEXT_IGNORE_PATTERNS, ...readContextIgnorePatterns(rootDir)];
}

function isReviewContextIgnored(rootDir, relativePath, { includeIgnored = false } = {}) {
  if (includeIgnored) return false;
  const candidate = normalizeRepoPath(relativePath);
  return contextIgnorePatterns(rootDir).some((pattern) => matchesContextIgnorePattern(candidate, pattern));
}

function listTrackedFiles(rootDir, { includeIgnored = false } = {}) {
  return git(rootDir, ['ls-files'])
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    .filter((relativePath) => !isSensitivePath(relativePath))
    .filter((relativePath) => !isReviewContextIgnored(rootDir, relativePath, { includeIgnored }));
}
...
function buildContextIndex(rootDir, { files, includeIgnored = false } = {}) {
  ensurePrivateRuntimeIgnores(rootDir);
  const hasExplicitFiles = files && files.length;
  const candidates = (hasExplicitFiles ? files : listTrackedFiles(rootDir, { includeIgnored }))
    .map((file) => String(file).trim())
    .filter(Boolean)
    .filter((file) => hasExplicitFiles || !isReviewContextIgnored(rootDir, file, { includeIgnored }));
  const summaries = [];
...
function createDiffHunkPack(rootDir, { runId, base = 'HEAD', profile, includeIgnored = false } = {}) {
  ...
  const parsedDiffFiles = parseDiff(diff).filter((file) => !isSensitivePath(file.path));
  const ignoredPaths = parsedDiffFiles
    .filter((file) => isReviewContextIgnored(rootDir, file.path, { includeIgnored }))
    .map((file) => file.path);
  const allParsedFiles = parsedDiffFiles
    .filter((file) => !isReviewContextIgnored(rootDir, file.path, { includeIgnored }));
...
function assessSmallTaskFastPath(rootDir, { base = 'HEAD', includeIgnored = false } = {}) {
  const diff = git(rootDir, ['diff', '--no-ext-diff', '--unified=0', base, '--']);
  const parsedFiles = parseDiff(diff).filter((file) => !isSensitivePath(file.path));
  const ignoredDiffPaths = parsedFiles
    .filter((file) => isReviewContextIgnored(rootDir, file.path, { includeIgnored }))
    .map((file) => file.path);
  const files = parsedFiles
    .filter((file) => !isReviewContextIgnored(rootDir, file.path, { includeIgnored }));
  const allUntrackedPaths = git(rootDir, ['ls-files', '--others', '--exclude-standard'])
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    .filter((filePath) => !filePath.startsWith('.ai-engineering-loop/'))
    .filter((filePath) => !isSensitivePath(filePath));
  const ignoredUntrackedPaths = allUntrackedPaths
    .filter((filePath) => isReviewContextIgnored(rootDir, filePath, { includeIgnored }));
  const untrackedPaths = allUntrackedPaths
    .filter((filePath) => !isReviewContextIgnored(rootDir, filePath, { includeIgnored }));
  const changedPaths = [...new Set([...files.map((file) => file.path), ...untrackedPaths])];
  const ignoredPaths = [...new Set([...ignoredDiffPaths, ...ignoredUntrackedPaths])];
...
function smartContextSummary(result) {
  if (result.pack) {
    return {
      ...
      ignoredPaths: result.pack.reviewDelta?.ignoredPaths?.length || 0,
