...
const CONTEXT_PROFILE_LIMITS = Object.freeze({
  lean: Object.freeze({
    maker: Object.freeze({ maxFiles: 8, maxFileBytes: 16_000, maxTotalBytes: 48_000, maxEstimatedTokens: 12_000 }),
    'devil-advocate': Object.freeze({ maxFiles: 6, maxFileBytes: 8_000, maxTotalBytes: 24_000, maxEstimatedTokens: 6_000 }),
    judge: Object.freeze({ maxFiles: 3, maxFileBytes: 4_000, maxTotalBytes: 12_000, maxEstimatedTokens: 3_000 })
  }),
  standard: Object.freeze({
    maker: Object.freeze({ maxFiles: 10, maxFileBytes: 24_000, maxTotalBytes: 72_000, maxEstimatedTokens: 18_000 }),
    'devil-advocate': Object.freeze({ maxFiles: 8, maxFileBytes: 12_000, maxTotalBytes: 48_000, maxEstimatedTokens: 12_000 }),
    judge: Object.freeze({ maxFiles: 3, maxFileBytes: 8_000, maxTotalBytes: 24_000, maxEstimatedTokens: 6_000 })
  }),
...
const STAGE_LIMITS = CONTEXT_PROFILE_LIMITS.lean;
...
function contextError(message, code = 'UNSAFE_CONTEXT', details = [], budget = null) {
  const err = new Error(message);
  err.code = code;
  if (details.length) err.details = details;
  if (budget) err.budget = budget;
  return err;
}
...
function redactSecrets(input) {
  ...
  replace(
    /(^|[^\w-])(--?(?:api[_-]?key|access[_-]?token|auth[_-]?token|token|secret|password|authorization)\s+)(?:"[^"]*"|'[^']*'|[^\s,;]+)/gi,
    'key-value-secret',
...
function redactHomePaths(input) {
  return String(input ?? '')
    ...
    .replace(/(?:file:\/\/)?\/home\/[A-Za-z0-9._-]+/g, '[REDACTED HOME]')
    .replace(/(?:file:\/\/)?\/root(?=\/|\\|$)/g, '[REDACTED HOME]')
    .replace(/(?:file:\/\/)?(?:\/private)?\/var\/folders\/[A-Za-z0-9._/-]+/g, '[REDACTED TMP]')
    .replace(/\b[A-Za-z]:\\Users\\[A-Za-z0-9._-]+/g, '[REDACTED HOME]')
    .replace(/\\\\[A-Za-z0-9._-]+\\[A-Za-z0-9._$-]+/g, '[REDACTED HOME]');
}
...
function contextLimitsForProfile(profile, stage) {
  const reviewProfile = normalizeReviewProfile(profile);
  const limits = CONTEXT_PROFILE_LIMITS[reviewProfile]?.[stage];
  if (!limits) {
    throw contextError(`Unknown context stage: ${stage}. Use ${Object.keys(STAGE_LIMITS).join('|')}`, 'UNKNOWN_CONTEXT_STAGE');
  }
  return { reviewProfile, limits };
}

function createContextPack(rootDir, { runId, stage, files, profile } = {}) {
  const policy = loadPolicy(rootDir);
  const { reviewProfile, limits } = contextLimitsForProfile(profile || policy.reviewProfile, stage);
  ...
  for (const requested of uniqueFiles) {
    ...
    if (remaining <= 0) {
      const budget = {
        ...
        requestedBytes: totalBytes + raw.length,
        files: [...budgetEntries, {
          path: safe.relative,
      ...
      throw contextError(
        `${stage} context exceeds its byte budget (${limits.maxTotalBytes.toLocaleString()}-byte limit; ${budget.requestedBytes.toLocaleString()} requested)`,
        'CONTEXT_BYTE_LIMIT',
        budget.files.map((entry) =>
          `${entry.path}: ${entry.estimatedTokens.toLocaleString()} estimated tokens, ${entry.includedBytes.toLocaleString()}/${entry.sourceBytes.toLocaleString()} bytes included`
        ),
        budget
      );
    }
    ...
    if (estimatedTokens + tokens > limits.maxEstimatedTokens) {
      ...
      const budget = {
        ...
        requestedTokens: estimatedTokens + tokens,
        files
      };
      throw contextError(
        `${stage} context exceeds its estimated token budget (${limits.maxEstimatedTokens.toLocaleString()}-token limit; ${budget.requestedTokens.toLocaleString()} requested)`,
        'CONTEXT_TOKEN_LIMIT',
        files.map((entry) =>
          `${entry.path}: ${entry.estimatedTokens.toLocaleString()} estimated tokens, ${entry.includedBytes.toLocaleString()}/${entry.sourceBytes.toLocaleString()} bytes included${entry.truncated ? ' (truncated)' : ''}`
        ),
        budget
      );
    }
