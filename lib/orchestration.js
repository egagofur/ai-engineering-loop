 ...
 * 2. Documentation, configuration (e.g. subagent: true), binary existence, or IPC message
 *    dispatch do NOT constitute LLM execution proof.
...
/**
 * 2. Standardized Capability Evidence Factory
 */
function createCapabilityEvidence({
  ...
  return {
    ...
    childConversationId,
    modelExecutionProven: Boolean(modelExecutionProven),
...
function isCapabilityProvenForMode(evidence, targetModeId) {
  ...
  if (evidence.classification === 'NOT_AN_AGENT_EXECUTION_CAPABILITY') {
    return false;
  }

  // Reject documentation-only or configuration-only claims
  if (evidence.isDocumentationOnly) {
    return false;
  }
  ...
  if (
    ...
    if (!evidence.modelExecutionProven) {
      return false;
    }
    if (!evidence.independentContextProven && targetModeId === 'TRUE_INDEPENDENT_AGENT') {
      return false;
    }
    if (evidence.historyInherited === true && targetModeId === 'TRUE_INDEPENDENT_AGENT') {
      return false;
    }
  }
 ...
 */
function formatExecutionReport({ selectedMode, capabilityEvidence = {} }) {
  ...
  if (isContextOnly) {
    header.push('Review Method: Clean-Slate Artifact Isolation Barrier (Single Session Review)');
  }
