...
function pressureRatio(used, limit) {
  if (!Number.isFinite(limit) || limit <= 0) return 0;
  return used / limit;
}

function budgetWarnings({ runPressure, dayPressure, reviewProfile }) {
  const warnings = [];
  if (runPressure >= 0.9) warnings.push(`run token usage is at ${Math.round(runPressure * 100)}%; finish or compact context before another model call`);
  else if (runPressure >= 0.75) warnings.push(`run token usage is at ${Math.round(runPressure * 100)}%; use unresolved findings and changed hunks only`);
  else if (runPressure >= 0.5) warnings.push(`run token usage is at ${Math.round(runPressure * 100)}%; avoid full logs and repeated source dumps`);

  if (dayPressure >= 0.9) warnings.push(`daily token usage is at ${Math.round(dayPressure * 100)}%; pause non-critical reviews`);
  else if (dayPressure >= 0.75) warnings.push(`daily token usage is at ${Math.round(dayPressure * 100)}%; prefer lean review context`);

  if (reviewProfile === 'lean' && (runPressure >= 0.75 || dayPressure >= 0.75)) {
    warnings.push('lean review profile is active; keep DA/Judge to focused deltas unless a blocker requires escalation');
  }
  if (reviewProfile === 'thorough') {
    warnings.push('thorough review profile increases context size; use only when the human explicitly accepts the token cost');
  }
...
function budgetStatus(rootDir, { runId, nodeId, estimatedTokens = 0, now = new Date() } = {}) {
  ...
  const pressure = {
    run: pressureRatio(projectedThisRun, policy.perRunTokenLimit),
    day: pressureRatio(projectedToday, policy.dailyTokenLimit)
  };
  const warnings = budgetWarnings({
    runPressure: pressure.run,
    dayPressure: pressure.day,
