...
function budgetStatus(rootDir, { runId, estimatedTokens = 0, now = new Date() } = {}) {
...
function recordTokenUsage(
  rootDir,
  { runId, inputTokens, outputTokens, model, now = new Date() } = {}
) {
  const run = resolveRun(rootDir, runId);
  ...
  return { entry, status: budgetStatus(rootDir, { runId: run.runId, now }) };
}
