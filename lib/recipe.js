...
function validateCondition(condition, location, errors) {
  ...
  if (!/^[a-z][a-zA-Z0-9_.-]{0,127}$/.test(String(condition.field || ''))) {
    errors.push(`${location}.field is invalid`);
  }
...
function validateRecipe(recipe, { mode } = {}) {
  ...
  const order = topologicalOrder(nodes, byId, errors);
  const terminals = nodes.filter((node) => ['report', 'delivery'].includes(node.type));
  for (const terminal of terminals) {
    if (nodes.some((node) => (node.dependsOn || []).includes(terminal.id))) {
...
function loadRecipe(rootDir, id) {
  ...
  return { recipe, source, path: recipePath(directory, id) };
}
