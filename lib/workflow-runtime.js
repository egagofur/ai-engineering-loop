...
function assertReady(workflow, nodeId) {
  ...
  if (runtimeNode.status !== NODE_STATES.READY) {
    const dependencies = (node.dependsOn || [])
      .map((dependencyId) => ({
        node: workflow.plan.nodes.find((candidate) => candidate.id === dependencyId),
        state: workflow.state.nodes[dependencyId]
      }))
      .filter(({ state }) => state && !SUCCESS_STATES.has(state.status));
    const details = dependencies.map(({ node: dependency, state }) =>
      `Prerequisite node '${dependency?.id || 'unknown'}' is ${state.status}${dependency?.displayName ? ` (${dependency.displayName})` : ''}.`
    );
    const readyApproval = dependencies.length === 1
      && dependencies[0].node?.type === 'approval'
      && dependencies[0].state.status === NODE_STATES.READY
      ? dependencies[0].node
      : null;
    if (readyApproval) {
      details.push(
        `Approve it explicitly with: ai-engineering-loop node approve ${readyApproval.id} --yes --by <name> --run ${workflow.run.runId}`
      );
    } else if (runtimeNode.reason) {
      details.push(`Reason: ${runtimeNode.reason}.`);
    }
    throw runtimeError(
      `Node ${nodeId} is ${runtimeNode.status}, not READY`,
      'NODE_NOT_READY',
      details
    );
  }
...
function startWorkflowNode(rootDir, nodeId, { runId, now = new Date() } = {}) {
  ...
  return withWorkflowLock(rootDir, resolved, () => {
    ...
    if (node.type === 'approval') {
      throw runtimeError(`Node ${nodeId} must use its dedicated approval command`, 'WRONG_NODE_EXECUTOR');
    }
...
function applyWorkflowGate(rootDir, gate, { runId, now = new Date(), transitionOptions = {} } = {}) {
  ...
  return withWorkflowLock(rootDir, resolved, () => {
    ...
    const before = workflow.state.nodes[node.id];
    if (before.status !== NODE_STATES.RUNNING) assertReady(workflow, node.id);
    ...
    let changes = nodeChanges(workflow, node.id, NODE_STATES.PASSED, {
      attempts: before.attempts + (before.status === NODE_STATES.READY ? 1 : 0),
      reason: null,
      activity: null
    }, now);
    const decisionGroup = gate === 'judge' ? loopForDecision(workflow.plan, node.id) : null;
    const loopChanges = [];
    if (decisionGroup && run.state === RUN_STATES.GOAL_FROZEN) {
      const currentLoop = workflow.state.loopGroups[decisionGroup.id];
      const nextIteration = currentLoop.iteration + 1;
      changes = changes.concat(resetLoopNodes(workflow, decisionGroup, nextIteration, now));
      loopChanges.push(loopChange(workflow, decisionGroup.id, {
        status: 'RUNNING',
        iteration: nextIteration,
        lastOutcome: decisionGroup.repeatOutcome
      }, now));
    } else if (decisionGroup && run.state === RUN_STATES.JUDGED_PASS) {
      loopChanges.push(loopChange(workflow, decisionGroup.id, {
        status: 'EXITED',
        lastOutcome: decisionGroup.exitOutcome
      }, now));
    } else if (gate === 'judge' && run.state === RUN_STATES.GOAL_FROZEN) {
    ...
    workflow = commitChanges(
      ...
      changes,
      {
        ...
        gate,
        ...(decisionGroup ? {
          groupId: decisionGroup.id,
          iteration: workflow.state.loopGroups[decisionGroup.id].iteration
        } : {})
      },
      now,
      false,
...
function cancelWorkflowRun(
  rootDir,
  { runId, actor = 'human', reason = 'Replaced by a new Goal', now = new Date() } = {}
) {
  const resolved = resolveRunId(rootDir, runId);
  return withWorkflowLock(rootDir, resolved, () => {
    let workflow = loadWorkflow(rootDir, resolved);
    if (!/^[A-Za-z0-9@._-]{1,64}$/.test(String(actor))) {
      throw runtimeError('Cancellation actor must be a short account identifier', 'INVALID_RUN_ACTOR');
    }
    const cleanReason = redactSecrets(String(reason || '').trim()).text;
    if (!cleanReason || cleanReason.length > 500) {
      throw runtimeError('Cancellation reason must be 1-500 characters', 'INVALID_CANCEL_REASON');
    }
    workflow.run = transitionRun(rootDir, resolved, RUN_STATES.CANCELLED, {
      gate: 'run-cancelled',
      reason: cleanReason,
      historyMetadata: { actor: String(actor) },
      now
    });
    const changes = [];
    for (const node of workflow.plan.nodes) {
    ...
    const loopChanges = (workflow.plan.loopGroups || [])
      .filter((group) => !['EXITED', 'STOPPED'].includes(workflow.state.loopGroups[group.id]?.status))
      .map((group) => loopChange(workflow, group.id, { status: 'STOPPED' }, now));
    workflow = commitChanges(
      rootDir,
      workflow,
      'RUN_CANCELLED',
      changes,
      { actor: String(actor), reason: cleanReason },
      now,
      false,
      loopChanges
    );
