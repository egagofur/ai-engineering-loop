...
function planRecipeImport(existingRecipe, importedRecipe, operation) {
  if (!['replace', 'merge', 'cancel'].includes(operation)) {
    throw builderError('Import operation must be replace, merge, or cancel', 'INVALID_IMPORT_OPERATION');
  }
  validateCandidate(existingRecipe);
  validateCandidate(importedRecipe);
  if (operation !== 'merge') return { operation, nodeIdMap: {}, edgeIdMap: {} };
  return { operation, ...planGraphMerge(existingRecipe, importedRecipe) };
}

function applyRecipeImport(existingRecipe, importedRecipe, plan) {
  if (!plan || !['replace', 'merge', 'cancel'].includes(plan.operation)) {
    throw builderError('A valid import preview is required', 'INVALID_IMPORT_PLAN');
  }
  if (plan.operation === 'cancel') return existingRecipe;
  if (plan.operation === 'replace') return migrateLegacyRecipe(importedRecipe);
  return mergeGraphs(existingRecipe, importedRecipe, plan);
}
...
function installRecipe(rootDir, candidatePath, { replace = false } = {}) {
  const candidate = migrateLegacyRecipe(readCandidate(rootDir, candidatePath));
