...
function saveGoalDraft(rootDir, runId, contract) {
  ...
  const value = { ...contract, schemaVersion: 1, runId };
  atomicWriteJson(goalPaths(rootDir, runId, run.goal?.version || 1).draft, value);
  return value;
}

function freezeGoal(rootDir, runId, { actor, now = new Date() } = {}) {
  const run = loadRun(rootDir, runId);
  ...
  let contract;
  try {
    contract = JSON.parse(fs.readFileSync(paths.draft, 'utf8'));
  } catch (cause) {
    throw goalError(`Goal draft is unavailable: ${cause.message}`, 'GOAL_DRAFT_MISSING');
  }
  ...
  const hash = hashFile(paths.version);
  const transitionOptions = {
    ...
    statePatch: {
      goal: { version, frozen: true, hash, actor: human, frozenAt: now.toISOString() }
    },
    historyMetadata: { actor: human, goalVersion: version, goalHash: hash },
    now
  };
  if (run.workflow) {
...
module.exports = { saveGoalDraft, freezeGoal, unfreezeGoal };
